...
engine = create_engine('mysql://root:123456@localhost/warehouse')
...
def getProd():
    try:
        t = engine.execute(
            "select a.prod_id,concat(a.prod_name,'|产家：',a.jiuzhuang,'|型号：',a.xilie) from prod a order by a.prod_name")
        prod_dict = []
        for i in t:
...
def getProdSale():
    try:
        sql = "select a.prod_id,concat(a.prod_name,'|产家：',a.jiuzhuang,'|剩余库存：',b.num) " \
...
def prodInStock(trade_name, prod_id, num, unit_price, remark, funding_pool_id, userId):
    ...
    sql = "insert into stock_change_log(prod_id,change_num,change_type,remark,change_user_id,change_time) " \
          "values (%s,%s,'产品入库','%s','%s',sysdate())" % (prod_id, num, remark, userId)
    engine.execute(sql)
    engine.execute('commit')
    print('setp4 资金核减')
...
def prodSaleStock(trade_name, prod_id, num, unit_price, remark, funding_pool_id, developer_info, userId):
    ...
    engine.execute('commit')
    sql = "insert into stock_change_log(prod_id,change_num,change_type,remark,change_user_id,change_time) " \
          "values (%s,%s,'产品销售','%s','%s',sysdate())" % (prod_id, (0 - int(num)), remark, userId)
    engine.execute(sql)
    engine.execute('commit')
    print('setp3 资金增加')
...
def expenTrade(trade_name, remark, fee, userId):
    ...
    sql = "insert into funding_change_log(funding_pool_id,change_price," \
          "change_type,remark,change_user_id,change_time) " \
          "values (%s,%s,'平账花费扣减资金','%s','%s',sysdate())" \
...
def getStockChangDetail():
    sql = "select t.* from (select a.trade_time,'产品入库' type,b.user_name," \
          ...
          "union all " \
          "select a.trade_time,'产品销售' type,b.user_name,c.prod_name,a.num,a.unit_price," \
