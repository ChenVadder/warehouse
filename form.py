...
class sale_trade_fields(FlaskForm):
    ...
    unit_price = StringField('销售单价：', validators=[Regexp(r'^-?([1-9]\d*\.\d*|0\.\d*[1-9]\d*|0?\.0+|0)$|^-?[1-9]\d*$',
                                                         message='销售单价必须为数字')])
    remark = TextAreaField('订单备注：')
    developer_info = TextAreaField('推介人信息：')
...
class add_prod_fields(FlaskForm):
    ...
    jiuzhuang = StringField('产家：', validators=[DataRequired(message='必须填写产家')])
    xilie = StringField('型号：', validators=[DataRequired(message='必须填写型号')])
    remark = TextAreaField('订单备注：')
